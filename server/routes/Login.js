...
const TEST_USER_USERNAME = "TestUsername";
    ...
    if (isFirstTime) {
      ...
      bcrypt.hash(googleId, saltRounds, (err, hash) => {
        db.query(
          ...
          //Start assigning default friends and friend requests
          ...
          //Finished assigning default friends and friend requests
...
router.post("/test", (req, res) => {
  ...
  const username =
    TEST_USER_USERNAME + Math.floor(Math.random() * 10 * Math.pow(10, 7));
  console.log(TEST_USER_PASSWORD);
  bcrypt.hash(password, saltRounds, (err, hash) => {
    const defaultSubscriptions = [
      {
        current_episode_air_date: "2011-04-17",
        current_episode_id: 63056,
        current_episode_name: "Winter Is Coming",
        current_episode_number: 1,
        current_episode_order: 5,
        current_season_id: 3624,
        current_season_name: "Season 1",
        show_id: 1399,
        show_title: "Game of Thrones",
      },
      {
        current_episode_air_date: "2008-01-20",
        current_episode_id: 62085,
        current_episode_name: "Pilot",
        current_episode_number: 1,
        current_episode_order: 1,
        current_season_id: 3572,
        current_season_name: "Season 1",
        show_id: 1396,
    ...
    db.query(
      "INSERT INTO users(username, password, profile_pic, subscriptions) values($1,$2,$3,$4) RETURNING *",
      [username, hash, defaultPic, JSON.stringify(defaultSubscriptions)],
      async (err, result) => {
        if (err) {
          console.log(err);
          ...
          await db.query("INSERT INTO roles VALUES ($1, $2)", [
            user_id,
            "Test User",
          ]);
...
router.delete("/test", verifyJWT, async (req, res) => {
  const userToBeDeleted = await db.query(
    "SELECT * FROM roles WHERE user_id=$1 AND role=$2",
    [req.userID, "Test User"]
  );
  if (userToBeDeleted.rows.length > 0) {
    await db.query("DELETE FROM users WHERE user_id=$1", [
      userToBeDeleted.rows[0].user_id,
    ]);
